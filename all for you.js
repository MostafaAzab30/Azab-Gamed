...
temp += `<div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 javababy mb-4" onclick="getdetails(${zed[i].id} )">
<div class="item g-4 dadgames">
<div class="small">
<img src="${zed[i].thumbnail}" class="w-100 rounded" alt="">
...
<p class="text-white">${zed[i].short_description.slice(1,150)}</p>

<br>
</div>

<div class="d-flex justify-content-space-between text-white names my-1 mt-3">
<h6 class="pull">${zed[i].genre}</h6>
<h6 class="pull">${zed[i].platform}</h6>
</div>
</div>
...
document.getElementById("addgames").innerHTML = temp
...
async function getdetails(hate) {
...
const api =await fetch(`https://free-to-play-games-database.p.rapidapi.com/api/game?id=${hate}`, options)
...
<div class="col-lg-8 col-sm-12 text-white lineheigh">
